...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        self.tiles = dict()
        all_xs = [i for i in range(int(self.width))]
        all_ys = [i for i in range(int(self.height))]
        for pos_x in all_xs:
            for pos_y in all_ys:
                self.tiles[(pos_x, pos_y)] = self.dirt_amount
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        tile_pos = (int(pos.get_x()), int(pos.get_y()))
        tile_dirt_capacity = self.tiles[tile_pos]
        after_clean = tile_dirt_capacity - capacity

        self.tiles[tile_pos] = after_clean if after_clean > 0 else 0
    ...
    def is_tile_cleaned(self, m, n):
        ...
        Note: The tile is considered clean only when the amount of dirt on this
              tile is 0.
        """
        return True if self.tiles[(m, n)] == 0 else False

    def get_num_cleaned_tiles(self):
        """
        Returns: an integer; the total number of clean tiles in the room
        """
        return sum([1 if tile_status == 0 else 0
                    for tile_status in self.tiles.values()])
    ...
    def is_position_in_room(self, pos):
        ...
        if pos_x < 0 or pos_x >= self.width \
                or pos_y < 0 or pos_y >= self.height:
            return False
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        random_room_pos_x = float(random.randint(0, self.room.width - 1))
        random_room_pos_y = float(random.randint(0, self.room.height - 1))
        self.robot_pos = Position(random_room_pos_x, random_room_pos_y)

        self.robot_dir = float(random.randint(0, 360 - 1))
...
class EmptyRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        random_room_pos_x = float(random.randint(0, self.width - 1))
        random_room_pos_y = float(random.randint(0, self.height - 1))
        pos = Position(random_room_pos_x, random_room_pos_y)
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_furnished(self, pos):
        ...
        """
        pos = (int(pos.get_x()), int(pos.get_y()))
        return True if pos in self.furniture_tiles else False
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        curr_angle = self.get_robot_direction()

        new_pos = curr_pos.get_new_position(curr_angle, self.speed)
        if self.room.is_position_valid(new_pos):
            ...
            self.set_robot_direction(float(random.randint(0, 360 - 1)))
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        ...
        robots = [robot_type(empty_room, speed, capacity)] * num_robots

        while coverage < min_coverage:
            time_step += 1
            for robot in robots:
                robot.update_position_and_clean()
                coverage = float(empty_room.get_num_cleaned_tiles()) \
                    / empty_room.get_num_tiles()
...
#       of a 20x20 room?
#   The performance shows that the StandardRobot relatively took lesser steps than the FaultyRobots. As the number of robots increase, the gap between the robots decreased.
#
# 2) How does the performance of the two robot types compare when two of each
#       robot cleans 80% of rooms with dimensions
#       10x30, 20x15, 25x12, and 50x6?
#   Both robots cleaned the room fastest when the ratio of the room is close to 1, which is 20x15 dimension. For FaultyRobot, steps at 10x30 dimension were 50 steps more than the 20x15. It approximately has increment of 50 steps every ratio after 20x15 dimension. For the StandardRobot, the slope is more even; yet it also shows an increase in steps as the ratio increases. 
#
