...
def get_word_score(word, n):
    ...
    first_comp = sum([SCRABBLE_LETTER_VALUES[char] for char in word_lower])
...
def is_valid_word(word, hand, word_list):
    ...
    if '*' in word_lower:
        possible_words = [word_lower.replace('*', vowel) for vowel in VOWELS]
        return any([True if i in word_list else False for i in possible_words])
    ...
    while calculate_handlen(hand) > 0:
        ...
        # Otherwise (the input is not two exclamation points):
            ...
            if is_valid_word(user_input, hand, word_list):
                ...
                print(f'"{user_input}" earned {score} points.'
            ...
            # update the hand by removing the letters of their input word
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    hand = deal_hand(num_hand)
    display_hand(hand)
    sub_flag = input('Would you like to substitute a letter? [yes/no]')

    if sub_flag.lower() in ('yes', 'no') and sub_flag.lower() == 'yes':
    ...
    replay_flag = input('Would you like to replay the hand? [yes/no]')
    if replay_flag.lower() == 'yes':
        total_score += play_hand(hand, word_list)
    print('---' * 10)
